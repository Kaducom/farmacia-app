...
function Notificacoes() {
  ...
  async function verificarAgora() {
    try {
      ...
      if (alertasMedicamentos.length > limite) {
        new Notification("Farmácia App", {
          body: `Existem mais ${
            alertasMedicamentos.length - limite
          } alerta(s) de medicamentos no app.`,
      ...
      mostrarToast(
        `${
          alertasMedicamentos.length + lembretesParaNotificar.length
        } aviso(s) verificado(s) 🔔`,
        "ok"
      );
    } catch (err) {
  ...
  function getAnoSemana(data) {
    ...
    const semana = Math.ceil((((copia - anoInicio) / 86400000) + 1) / 7);
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      ...
      <div className="relative z-10 mx-auto max-w-5xl space-y-5 p-4 pb-32 text-gray-950 dark:text-white">
        ...
        <div className="rounded-3xl border border-blue-200 bg-blue-50/90 p-5 text-blue-800 shadow-xl backdrop-blur-xl dark:border-blue-500/20 dark:bg-blue-500/10 dark:text-blue-300">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm">
                Esta fase verifica alertas quando o app está aberto ou quando
                você toca em verificar agora. Depois dá pra evoluir para
                notificação em segundo plano com service worker.
              </p>
            </div>
          </div>
        </div>
...
function HeaderNotificacoes({ resumo, permissao, aba, setAba }) {
  const perigo = resumo.vencidos + resumo.remover;
  const atencao = resumo.pre + resumo.proximos + resumo.lembretesVencidos;
  ...
  return (
    <div className="overflow-hidden rounded-[2rem] border border-white/70 bg-white/90 shadow-2xl shadow-black/5 backdrop-blur-2xl dark:border-white/10 dark:bg-gray-950/75">
      <div className="relative overflow-hidden bg-gradient-to-br from-amber-600 via-orange-700 to-slate-950 p-6 text-white">
        ...
        <div className="relative flex items-center gap-4">
          ...
          <div className="min-w-0 flex-1">
            <div className="mb-2 flex flex-wrap items-center gap-2">
              <span className="inline-flex items-center gap-1 rounded-full bg-white/15 px-2.5 py-1 text-[11px] font-black uppercase tracking-wide text-amber-50">
                <Sparkles size={13} />
                Central de alertas
              </span>

              <BadgePermissaoMini permissao={permissao} />
            </div>

            <h1 className="text-2xl font-black">Notificações</h1>

            <p className="mt-1 text-sm text-amber-100">
              Validade, remoção, pré-vencimento e lembretes de seção.
            </p>
          </div>
        </div>
        ...
        <div className="relative mt-6 grid grid-cols-4 gap-2 sm:gap-3">
          ...
          <ResumoTopo icon={Skull} valor={perigo} label="Crítico" />
          <ResumoTopo icon={Bell} valor={atencao} label="Atenção" />
          <ResumoTopo icon={ClipboardList} valor={resumo.lembretes} label="Rotina" />
        </div>
      </div>
...
function PermissaoCard({ permissao, pedirPermissao, enviarTeste }) {
  return (
    <div className="rounded-[2rem] border border-gray-200 bg-white/90 p-5 shadow-xl backdrop-blur-xl dark:border-white/10 dark:bg-gray-950/75">
      ...
      <div className="mt-5 grid grid-cols-2 gap-3">
        <button
          ...
          className="flex h-12 items-center justify-center gap-2 rounded-2xl bg-amber-600 font-black text-white shadow-lg shadow-amber-600/20 transition active:scale-95"
        >
          <Bell size={19} />
          Ativar
        </button>
        ...
        <button
          type="button"
          onClick={enviarTeste}
          className="flex h-12 items-center justify-center gap-2 rounded-2xl bg-blue-600 font-black text-white shadow-lg shadow-blue-600/20 transition active:scale-95"
        >
          <BellRing size={19} />
          Teste
        </button>
  ...
  const alertasFiltrados = useMemo(() => {
    ...
    if (filtro === "criticos") {
      return alertas.filter((item) =>
        ["vencido", "remover"].includes(item.status)
      );
    }

    return alertas.filter((item) => item.status === filtro);
  }, [alertas, filtro]);
    ...
    <>
      <div className="rounded-[2rem] border border-gray-200 bg-white/90 p-5 shadow-xl backdrop-blur-xl dark:border-white/10 dark:bg-gray-950/75">
        <div className="mb-5 flex items-start justify-between gap-3">
          ...
          <button
            ...
            className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-amber-600 text-white transition active:scale-95 disabled:opacity-60"
          >
            {carregando ? (
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          <button
            ...
            className="flex h-12 items-center justify-center gap-2 rounded-2xl bg-emerald-700 font-black text-white shadow-lg shadow-emerald-700/20 transition active:scale-[0.98] disabled:opacity-60"
          >
            {processando ? (
              <Loader2 size={20} className="animate-spin" />
            ) : (
          ...
          <button
            type="button"
            onClick={() => setMostrarRegras((prev) => !prev)}
            className="flex h-12 items-center justify-center gap-2 rounded-2xl bg-gray-100 font-black text-gray-700 transition active:scale-[0.98] dark:bg-white/10 dark:text-gray-200"
          >
      ...
      <AnimatePresence>
        {mostrarRegras && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 14 }}
            className="rounded-[2rem] border border-gray-200 bg-white/90 p-5 shadow-xl backdrop-blur-xl dark:border-white/10 dark:bg-gray-950/75"
          >
            <div className="mb-5">
              <h2 className="flex items-center gap-2 text-lg font-black">
                <Settings size={21} />
            ...
            <label className="block">
              <span className="mb-2 block text-sm font-black text-gray-700 dark:text-gray-200">
              ...
              <input
                ...
                className="h-12 w-full rounded-2xl border border-gray-200 bg-gray-50 px-4 font-black text-gray-900 outline-none transition focus:border-amber-500 focus:ring-4 focus:ring-amber-500/20 dark:border-white/10 dark:bg-white/10 dark:text-white"
              />
            </label>
            ...
            <button
              ...
              className="mt-5 flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-amber-600 font-black text-white shadow-lg shadow-amber-600/20 transition active:scale-[0.98]"
            >
              <Save size={20} />
              Salvar regras
            </button>
      ...
      <div className="rounded-[2rem] border border-gray-200 bg-white/90 p-5 shadow-xl backdrop-blur-xl dark:border-white/10 dark:bg-gray-950/75">
        <div className="mb-4 flex items-start justify-between gap-3">
          ...
          <span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700 dark:bg-amber-500/15 dark:text-amber-300">
        ...
        <div className="mb-5 flex gap-2 overflow-x-auto pb-1">
          ...
          />
          <FiltroChip
            ativo={filtro === "remover"}
            onClick={() => setFiltro("remover")}
            label="Remover"
          />
          <FiltroChip
            ativo={filtro === "pre"}
            onClick={() => setFiltro("pre")}
            label="Pré"
          />
          <FiltroChip
            ativo={filtro === "proximo"}
        ...
        {carregando ? (
          <div className="flex flex-col items-center justify-center rounded-3xl bg-gray-100 p-8 text-center dark:bg-white/5">
            <Loader2 size={32} className="mb-3 animate-spin text-amber-600" />
            <p className="font-black">Carregando alertas...</p>
          </div>
          ...
          <div className="rounded-3xl bg-emerald-50 p-8 text-center text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300">
            ...
            <p className="mt-1 text-sm">
              Nenhum medicamento entrou nesse filtro de alerta.
            </p>
          </div>
        ) : (
...
function AbaLembretes({
  ...
  const ordenados = useMemo(() => {
    return [...lembretes].sort((a, b) => {
      ...
      const ana = analisarLembrete(a);
      const anb = analisarLembrete(b);

      return (peso[ana.status] ?? 9) - (peso[anb.status] ?? 9);
    });
  }, [lembretes, analisarLembrete]);
      ...
      <div className="rounded-[2rem] border border-gray-200 bg-white/90 p-5 shadow-xl backdrop-blur-xl dark:border-white/10 dark:bg-gray-950/75">
        ...
        >
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              Olhar seção, conferir geladeira ou revisar prateleira.
            </p>
          </div>
          ...
          {mostrarFormulario && (
            ...
            >
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <label>
                  ...
                  <select
                    ...
                    className="h-12 w-full rounded-2xl border border-gray-200 bg-gray-50 px-4 font-black outline-none focus:border-amber-500 focus:ring-4 focus:ring-amber-500/20 dark:border-white/10 dark:bg-white/10"
                  >
              ...
              <button
                ...
                className="flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-amber-600 font-black text-white shadow-lg shadow-amber-600/20 transition active:scale-[0.98]"
              >
                <Plus size={20} />
                Criar lembrete
              </button>
      ...
      <div className="rounded-[2rem] border border-gray-200 bg-white/90 p-5 shadow-xl backdrop-blur-xl dark:border-white/10 dark:bg-gray-950/75">
        <div className="mb-5 flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              Rotina da farmácia sem depender da memória.
            </p>
          </div>
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="h-12 w-full rounded-2xl border border-gray-200 bg-gray-50 px-4 font-black outline-none placeholder:text-gray-400 focus:border-amber-500 focus:ring-4 focus:ring-amber-500/20 dark:border-white/10 dark:bg-white/10 dark:text-white"
      />
    </label>
  );
}

function AbaBotao({ ativa, onClick, icon: Icon, label }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
function FiltroChip({ ativo, onClick, label }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`
        inline-flex shrink-0 items-center gap-1.5 rounded-full px-3 py-2 text-xs font-black transition active:scale-95
        ${
          ativo
            ? "bg-amber-600 text-white shadow-lg shadow-amber-600/20"
...
function BadgePermissaoMini({ permissao }) {
  ...
  return (
    <span
      ...
      `}
    >
      {ok ? "Permitida" : negada ? "Bloqueada" : "Pendente"}
    </span>
  );
}
...
function ToggleLinha({ titulo, descricao, ativo, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex w-full items-center justify-between gap-3 rounded-2xl bg-gray-100 p-4 text-left transition active:scale-[0.98] dark:bg-white/5"
    >
      <div>
...
function CardAlerta({ item, formatarData }) {
  const config = {
    vencido: {
      ...
      borda: "border-red-200 dark:border-red-500/20",
    },
    remover: {
      icon: AlertTriangle,
      cor: "bg-orange-600",
      ...
      borda: "border-orange-200 dark:border-orange-500/20",
    },
    pre: {
      icon: Clock3,
      cor: "bg-yellow-500",
      ...
      borda: "border-yellow-200 dark:border-yellow-500/20",
    },
    proximo: {
      icon: Bell,
      cor: "bg-blue-600",
      ...
      borda: "border-blue-200 dark:border-blue-500/20",
    },
  };
  ...
  return (
    <motion.div
      ...
      initial={{ opacity: 0, y: 12 }}
      animate={{ opacity: 1, y: 0 }}
      className={`rounded-3xl border p-4 ${info.fundo} ${info.borda}`}
    >
      <div className="flex items-start gap-3">
        <div
          className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl text-white shadow-lg ${info.cor}`}
        >
          <Icon size={21} />
        </div>
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="break-words font-black text-gray-950 dark:text-white">
                {item.med.nome || "Medicamento sem nome"}
              </p>
            </div>

            <span className="shrink-0 rounded-full bg-white/70 px-3 py-1 text-xs font-black text-gray-700 dark:bg-gray-900/60 dark:text-gray-200">
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-xs font-bold text-gray-500 dark:text-gray-400">
            ...
            {item.dataRemover && (
              <span className="rounded-full bg-white/70 px-3 py-1 dark:bg-gray-900/60">
                Remover: {formatarData(item.dataRemover)}
              </span>
            )}
          </div>
        </div>
  ...
  return (
    <motion.div
      ...
      className={`
        ...
        ${
          !ativo
            ? "border-gray-200 bg-gray-100 opacity-75 dark:border-white/10 dark:bg-white/5"
            : vencido
            ? "border-amber-200 bg-amber-50 dark:border-amber-500/20 dark:bg-amber-500/10"
            : feito
    ...
    >
      <div className="flex items-start gap-3">
        <div
          ...
          `}
        >
          {feito ? <CheckCircle2 size={21} /> : <Clock3 size={21} />}
        </div>
...
function Toast({ toast, fechar }) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      exit={{ opacity: 0, y: -14 }}
      className="fixed left-1/2 top-5 z-[100000] w-[92%] max-w-sm -translate-x-1/2"
    >
      <div
        className={`
